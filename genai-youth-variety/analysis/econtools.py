...
def absorbed_2sls(df, y, endog, exog, instr, absorb=("firm", "year"),
    ...
    extra = dict(first_stage_F=min(fstats.values()),
                 ...
                 first_stage_all=fcoef, first_stage_F_each=fstats,
...
# ---------------------------------------------------------------------------
def _fast_within(M, fid, tid, iters=25, tol=1e-10):
    """Alternating projections onto the firm and year mean spaces."""
    M = np.array(M, float, copy=True)
    nf, nt = int(fid.max()) + 1, int(tid.max()) + 1
    cf = np.bincount(fid, minlength=nf).astype(float)
    ct = np.bincount(tid, minlength=nt).astype(float)
    cf[cf == 0] = 1.0
    ct[ct == 0] = 1.0
    for _ in range(iters):
        big = 0.0
        for j in range(M.shape[1]):
            col = M[:, j]
            m = np.bincount(fid, weights=col, minlength=nf) / cf
            col -= m[fid]
            m = np.bincount(tid, weights=col, minlength=nt) / ct
            col -= m[tid]
            big = max(big, float(np.abs(m).max()))
            M[:, j] = col
        if big < tol:
            break
    return M
...
# regime switching
# --------------------------------------------------------------------------
def panel_threshold(df, y, key, q, controls, entity="firm", time="year",
                    trim=0.15, grid=200, n_boot=500, seed=23,
                    cluster="firm"):
    """Hansen (1999) fixed-effects panel threshold regression.

    The slope on `key` is allowed to differ across regimes defined by whether
    the threshold variable `q` lies below or above an unknown value gamma:

        y_it = beta_1 key_it 1{q_it <= gamma} + beta_2 key_it 1{q_it > gamma}
               + controls + mu_i + lambda_t + e_it

    gamma is estimated by concentrated least squares over a trimmed grid of
    the empirical quantiles of q.  Writing the model as a regression of y on
    key, on key x 1{q <= gamma} and on the controls turns the grid search into
    a sequence of one-regressor problems, which is what makes the bootstrap
    affordable.  The null of no threshold is tested by the likelihood-ratio
    statistic F1 = (SSR_0 - SSR_1) / sigma^2, whose limiting distribution is
    non-standard; its p-value comes from the fixed-regressor bootstrap of
    Hansen (1996).  The confidence interval for gamma inverts the sequence of
    likelihood ratios at the asymptotic critical value of Hansen (1999).
    """
    cols = [y, key, q] + list(controls)
    d = df.dropna(subset=cols + [entity, time]).copy().reset_index(drop=True)
    fid = pd.factorize(d[entity])[0]
    tid = pd.factorize(d[time])[0]

    qv = d[q].to_numpy(float)
    kv = d[key].to_numpy(float)
    lo, hi = np.quantile(qv, [trim, 1 - trim])
    cand = np.unique(np.quantile(qv[(qv >= lo) & (qv <= hi)],
                                 np.linspace(0.0, 1.0, grid)))
    G = cand.size

    # one within transform for everything: outcome, key, controls and every
    # candidate regime column
    Z = kv[:, None] * (qv[:, None] <= cand[None, :])
    M = np.column_stack([d[y].to_numpy(float), kv,
                         d[list(controls)].to_numpy(float), Z])
    W = _fast_within(M, fid, tid)
    yv = W[:, 0]
    A = W[:, 1:2 + len(controls)]          # key and the controls
    Zw = W[:, 2 + len(controls):]

    # partial the fixed regressors out of the outcome and of every candidate
    AtA_inv = np.linalg.pinv(A.T @ A)
    ya = yv - A @ (AtA_inv @ (A.T @ yv))
    Za = Zw - A @ (AtA_inv @ (A.T @ Zw))
    zz = np.einsum("ij,ij->j", Za, Za)
    zz = np.where(zz <= 1e-10, np.nan, zz)

    ssr0 = float(ya @ ya)
    num = (ya @ Za) ** 2 / zz
    ssrs = ssr0 - num
    jbest = int(np.nanargmin(ssrs))
    gamma = float(cand[jbest])
    ssr1 = float(ssrs[jbest])
    n = len(d)
    sig2 = ssr1 / n
    f1 = (ssr0 - ssr1) / sig2

    # fixed-regressor bootstrap under the null of no threshold
    e0 = ya - Za[:, jbest] * 0.0            # restricted residual is ya itself
    rng = np.random.default_rng(seed)
    draws = np.empty(n_boot)
    for r in range(n_boot):
        yb = e0 * rng.standard_normal(n)
        yb = yb - A @ (AtA_inv @ (A.T @ yb))
        s0 = float(yb @ yb)
        s1 = np.nanmin(s0 - (yb @ Za) ** 2 / zz)
        draws[r] = (s0 - s1) / (s1 / n)
    p_boot = float((draws >= f1).mean())

    # confidence interval for gamma by inverting the likelihood ratio
    lr = (ssrs - ssr1) / sig2
    crit = -2.0 * np.log(1.0 - np.sqrt(0.95))
    inside = cand[np.nan_to_num(lr, nan=np.inf) <= crit]
    ci = (float(inside.min()), float(inside.max())) if inside.size else \
        (float("nan"), float("nan"))

    # regime coefficients with firm-clustered standard errors
    d1 = (qv <= gamma).astype(float)
    Mf = np.column_stack([d[y].to_numpy(float), kv * d1, kv * (1.0 - d1),
                          d[list(controls)].to_numpy(float)])
    Wf = _fast_within(Mf, fid, tid)
    yf, Xf = Wf[:, 0], Wf[:, 1:]
    bf, *_ = np.linalg.lstsq(Xf, yf, rcond=None)
    ef = yf - Xf @ bf
    names = [key + "_low", key + "_high"] + list(controls)
    V, ncl = _cluster_cov(Xf, ef, d[cluster].to_numpy(), dof_extra=len(names))
    se = np.sqrt(np.diag(V))
    tst = bf / se
    pv = 2 * (1 - stats.t.cdf(np.abs(tst), max(ncl - 1, 1)))
    sst = float((_fast_within(d[[y]].to_numpy(float), fid, tid)[:, 0]
                 ** 2).sum())
    res = Res(pd.Series(bf, index=names), pd.Series(se, index=names),
              pd.Series(tst, index=names), pd.Series(pv, index=names),
              n, 1.0 - float(ef @ ef) / sst)
    res.extra = dict(gamma=gamma, ci_low=ci[0], ci_high=ci[1], f1=float(f1),
                     p_boot=p_boot, n_boot=int(n_boot), ssr0=ssr0, ssr1=ssr1,
                     grid=[float(x) for x in cand],
                     lr=[float(x) for x in np.nan_to_num(lr, nan=np.inf)],
                     crit=float(crit), n_clusters=int(ncl),
                     share_low=float(d1.mean()))
    return res
