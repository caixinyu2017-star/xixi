...
def para(text, red=True, indent=480, jc=None, bold=False, sz=None):
    ppr = ['<w:pPr>']
    ...
    return X('<ROOT><w:p>%s%s</w:p></ROOT>'
             % (''.join(ppr), _runs(text, red=red, bold=bold, sz=sz)))[0]


def heading(text, level=2, red=True, numbered=True):
    nonum = '' if numbered else '<w:numPr><w:ilvl w:val="0"/><w:numId w:val="0"/></w:numPr>'
    jc = '' if numbered else '<w:jc w:val="center"/>'
    ind = '' if numbered else '<w:ind w:firstLineChars="0" w:firstLine="0"/>'
    if level == 2:
        ppr = ('<w:pPr><w:pStyle w:val="2"/>' + nonum + ind + jc +
               '<w:rPr>%s<w:lang w:eastAsia="zh-CN"/></w:rPr></w:pPr>')
    else:
        ppr = ('<w:pPr><w:pStyle w:val="a"/>' + nonum + ind + jc +
               '<w:rPr>%s<w:lang w:eastAsia="zh-CN"/></w:rPr></w:pPr>')
...
def caption(kind, chap, num, text, red=True):
    ...
    ppr = ('<w:pPr><w:pStyle w:val="af7"/>%s<w:ind w:firstLineChars="0" w:firstLine="0"/>'
...
def table(header, rows, red=True, total_w=8296):
    ...
    weights = [m ** 0.70 for m in maxlen]
    tot = sum(weights)
    widths = [max(1500, int(total_w * w / tot)) for w in weights]
    # 归一化回 total_w
    ...
    xml = ('<w:tbl><w:tblPr><w:tblStyle w:val="SanXian"/>'
