...
if errorlevel 1 wsl.exe --install -d %DISTRO%
if errorlevel 1 (
    echo.
...
if not errorlevel 1 goto have_distro
echo           正在安装 Ubuntu 24.04，需要下载数百 MB，请耐心等待...
echo.
echo --------------------------------------------------
echo  注意：稍后会要求创建 Linux 用户，请照做：
...
:::if errorlevel 1 (
:::    echo [出错] 尚未安装 Claude Science ，请先运行 ClaudeScience-Install.bat 完成安装。
:::    pause
:::    exit /b 1
:::)
