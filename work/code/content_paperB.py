...
def blocks(st):
    ...
    ap({'p': (
        ...
        'customer base into relatively homogeneous groups, is a prerequisite for market positioning, precision '
    ...
    ap({'p': (
        ...
        '$t$, $r$ is a uniform random number in [0, 1] drawn once per individual, $rand$ is a uniform '
        'random number in [0, 1] drawn independently for each dimension, $p=0.9$ is the switching threshold, '
    ...
    ap({'p': (
        'where the control parameter $a$ is set to 0.7 in this paper. In the implementation, a tiny random '
        'perturbation on the order of $10^{-12}$ is added to the chaotic variable after each iteration '
        '(modulo 1) to avoid the fixed points and short periodic cycles of the tent map, and zero initial '
        'values are replaced by a non-zero constant. Figure 2 compares the initial '
...
def blocks_experiments(st):
    ...
    ap({'p': (
        ...
        'dimensions and 20,000 for 20 dimensions. Since $MaxFEs$ serves as the stopping criterion, the '
        'nominal maximum iteration number of MSBKA is $T=\\lfloor MaxFEs/(2N+1) \\rfloor$ and the ratio '
        '$t/T$ is truncated to 1 when necessary. The population size of all algorithms was 30, and '
    ...
    ap({'p': (
        ...
        'right-skewed, a base-10 logarithmic transformation $x\\rightarrow \\log_{10}(x+1)$ was applied to '
        'each spending value before clustering, and the number of '
