# -*- coding: utf-8 -*-
"""Paper 6 display equations in OMML."""
...
# (4) Y_i = β_0 + β_1 FACT_{LDC,i} + ε_i
EQ4 = omath(
    msub(mr('Y'), mr('i')) + _op('=') + msub(mr('β'), mr('0')) + _op('+') +
    msub(mr('β'), mr('1')) + _sp() + msub(mr('FACT', upright=True), mr('LDC,i')) +
    _op('+') + msub(mr('ε'), mr('i'))
)
...
EQUATIONS_P6 = {'eq1': EQ1, 'eq2': EQ2, 'eq3': EQ3, 'eq4': EQ4, 'eq5': EQ5}
