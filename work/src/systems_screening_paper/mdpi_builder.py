...
def mr(t, upright=False, bold=False):
    ...
    Operator names such as min/max/ln render correctly in both Word and
    LibreOffice when emitted as a single *upright* run, so no splitting is
    needed there. Italic runs whose text collides with a StarMath keyword are
    still split, because LibreOffice's importer would otherwise turn them into
    symbols (e.g. "in" into the set-membership sign)."""
    if (not upright and t and t.strip().lower() in _LO_MATH_KEYWORDS
            and len(t.strip()) > 1):
        s = t.strip()
...
def mdelim2(a, b, left='(', right=')', sep='|'):
    """Two-argument delimiter, e.g. a conditional density f(x|y). The bar is
    the delimiter's own separator character rather than a text run, which is
    what Word writes and what non-Word readers import correctly; a literal '|'
    run is re-interpreted as a logical operator by some importers."""
    return (f'<m:d><m:dPr><m:begChr m:val="{left}"/><m:sepChr m:val="{sep}"/>'
            f'<m:endChr m:val="{right}"/></m:dPr>'
            f'<m:e>{a}</m:e><m:e>{b}</m:e></m:d>')
...
_GREEK = {'alpha':'α','beta':'β','gamma':'γ','delta':'δ','epsilon':'ε',
          ...
          'tau':'τ','nu':'ν','sigma':'σ','rho':'ρ','pi':'π','chi':'χ',
          'psi':'ψ','xi':'ξ','zeta':'ζ','iota':'ι','upsilon':'υ'}

_MRPR_RE = re.compile(r'(<m:rPr/>|<m:rPr>.*?</m:rPr>)')

def _size_math(omml_xml, sz):
    """Give every OMML run the surrounding text's point size, so that inline
    math does not render larger than the sentence it sits in."""
    wrpr = (f'<w:rPr>{PALATINO}<w:sz w:val="{sz}"/>'
            f'<w:szCs w:val="{sz}"/></w:rPr>')
    return _MRPR_RE.sub(lambda m: m.group(1) + wrpr, omml_xml)
...
def p_table_caption(num, text, refmap=None):
    inner = (wrun(f'Table {num}. ', sz=17, b=True)
             + rich_runs(text, refmap or {}, sz=17))
    return para(inner, '<w:keepNext/><w:spacing w:before="160" w:after="40"/>')

def p_fig_caption(num, text, refmap=None):
    inner = (wrun(f'Figure {num}. ', sz=17, b=True)
             + rich_runs(text, refmap or {}, sz=17))
...
def p_reference(num, authors, title, journal, year, tail):
    ...
    tsep = ' ' if title.rstrip().endswith(('?', '!', '…')) else '. '
    runs = [wrun(f'{num}. {authors} {title}{tsep}', sz=16),
...
def _tc(text_lines, width, borders, bold=False, align='center', sz=16):
    ...
    for ln in (text_lines if isinstance(text_lines, list) else [text_lines]):
        t = str(ln)
        if IMATH_RE.search(t):
            runs = rich_runs(t, {}, sz=sz, b=bold)
        else:
            runs = wrun(t, sz=sz, b=bold) if t else ''
...
SECT_PR = ('<w:sectPr><w:headerReference r:id="rId7" w:type="first"/>'
           ...
           '<w:lnNumType w:countBy="1" w:distance="170" w:restart="continuous"/>'
