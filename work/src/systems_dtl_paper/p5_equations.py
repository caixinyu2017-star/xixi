# -*- coding: utf-8 -*-
"""Paper 5 display equations in OMML."""
...
# (4) Y_i = β_0 + β_1 FACT_{DLDM,i} + ε_i
EQ4 = omath(
    msub(mr('Y'), mr('i')) + _op('=') + msub(mr('β'), mr('0')) + _op('+') +
    msub(mr('β'), mr('1')) + _sp() + msub(mr('FACT', upright=True), mr('DLDM,i')) +
    _op('+') + msub(mr('ε'), mr('i'))
)

# (5) W = Σ_{c=1}^{K} Σ_{i∈C_c} ‖ z_i − μ_c ‖²
_norm = mdelim(msub(mr('z'), mr('i')) + _op('−') + msub(mr('μ'), mr('c')),
               left='‖', right='‖')
EQ5 = omath(
    mr('W') + _op('=') +
    msum(mr('c=1'), mr('K'),
         msum(mr('i∈') + msub(mr('C'), mr('c')), '',
              msup(_norm, mr('2'))))
)

EQUATIONS_P5 = {'eq1': EQ1, 'eq2': EQ2, 'eq3': EQ3, 'eq4': EQ4, 'eq5': EQ5}
