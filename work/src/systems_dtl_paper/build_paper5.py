# -*- coding: utf-8 -*-
"""Assemble the paper-5 manuscript docx (digital transformation leadership ×
...
from p5_equations import EQUATIONS_P5
from p5_content_a import PART_A, TITLE, ABSTRACT, KEYWORDS
from p5_content_b import PART_B
from p5_refs import REFS5 as REFS
...
# ------------------------------------------------------------------ body
FIG_RIDS = {f'p5_fig{i}.png': f'rId{19 + i}' for i in range(1, 6)}
EMU_PER_IN = 914400
...
for b in BLOCKS:
    ...
    if kind == 'h1':
        ...
        parts.append(p_equation(EQUATIONS_P5[b[1]], b[2]))
    elif kind == 'tabcap':
...
out = build_docx(os.path.join(SCRATCH, 'template_scaffold.docx'),
                 os.path.join(SCRATCH, 'DTL_Youth_Employment_Systems.docx'),
                 body_xml,
                 extra_media={f'p5_fig{i}.png': os.path.join(SCRATCH, f'p5_fig{i}.png')
                              for i in range(1, 6)})
