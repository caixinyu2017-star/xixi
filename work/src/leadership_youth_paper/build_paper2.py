# -*- coding: utf-8 -*-
"""Assemble the paper-2 manuscript docx (digital leadership × youth employment, EU-27)."""
import os, re, sys
...
from p2_equations import EQUATIONS_P2
from p2_content_a import PART_A, TITLE, ABSTRACT, KEYWORDS
from p2_content_b import PART_B
from paper_refs import REFS as REFS1
from p2_refs import REFS2
...
# ------------------------------------------------------------------ body
FIG_RIDS = {f'p2_fig{i}.png': f'rId2{i-1}' for i in range(1, 6)}
parts = [front]
for b in BLOCKS:
    kind = b[0]
    if kind == 'h1':
        ...
        parts.append(p_equation(EQUATIONS_P2[b[1]], b[2]))
    elif kind == 'tabcap':
...
out = build_docx(os.path.join(SCRATCH, 'template_scaffold.docx'),
                 os.path.join(SCRATCH, 'Digital_Leadership_Youth_Employment_Systems.docx'),
                 body_xml,
                 extra_media={f'p2_fig{i}.png': os.path.join(SCRATCH, f'p2_fig{i}.png')
                              for i in range(1, 6)})
print('built:', out)
