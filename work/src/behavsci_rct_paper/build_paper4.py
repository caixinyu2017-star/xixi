# -*- coding: utf-8 -*-
"""Assemble the paper-4 manuscript docx (CareerMate RCT) on the official MDPI
Behavioral Sciences template."""
import os, re, struct, sys, unicodedata
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from bs_builder import (CITE_RE, build_docx, p_style, p_h1, p_h2, p_h3, p_body, p_quote,
...
from p4_equations import EQUATIONS_P4 as EQUATIONS_P3
from p4_content_a import PART_A, TITLE, ABSTRACT, KEYWORDS
from p4_content_b import PART_B
from p4_refs import REFS4 as REFS3
...
FIG_WIDTH_IN = {'p4_fig1.png': 6.30, 'p4_fig2.png': 6.60, 'p4_fig3.png': 5.90,
                'p4_fig4.png': 6.80}
FIG_RIDS = {f'p4_fig{i}.png': f'rId{19 + i}' for i in range(1, 5)}
...
for b in BLOCKS:
    ...
    if kind == 'h1':
        ...
        parts.append(p_quote(b[1], REFS3))
    elif kind == 'eq':
...
out = build_docx(os.path.join(HERE, 'behavsci_template.dot'),
                 os.path.join(HERE, 'CareerMate_RCT_BehavSci.docx'),
                 ''.join(parts),
