# -*- coding: utf-8 -*-
"""Assemble the paper-7 manuscript docx (entry-level automation and the
expertise pipeline; enterprise survey + system dynamics)."""
import os, re, sys
...
from p7_equations import EQUATIONS_P7
from p7_content_a import PART_A, TITLE, ABSTRACT, KEYWORDS
from p7_content_b import PART_B
from p7_refs import REFS7 as REFS

SCRATCH = os.path.dirname(os.path.abspath(__file__))
BLOCKS = PART_A + PART_B
FIGS = [f'p7_fig{i}.png' for i in range(1, 8)]
...
print('2023+ share: %d/%d = %.0f%%' % (recent, len(key_order),
                                       100 * recent / len(key_order)))
...
def smart_table(rows, widths, repeat_header=True):
    ...
    for ri, row in enumerate(rows):
        ...
        trpr = ('<w:trPr><w:cantSplit/>'
                + ('<w:tblHeader/>' if header and repeat_header else '')
...
for b in BLOCKS:
    ...
    if kind == 'h1':
        ...
        parts.append(p_equation(EQUATIONS_P7[b[1]], b[2]))
    elif kind == 'tabcap':
        parts.append(p_table_caption(b[1], b[2], REFMAP))
    elif kind == 'table':
        ...
        parts.append(p_fig_caption(b[1], b[2], REFMAP))
    elif kind == 'back':
...
body_xml = ''.join(parts)
OUT_NAME = 'Vanishing_First_Rung_Expertise_Pipeline_Systems.docx'
out = build_docx(os.path.join(SCRATCH, 'template_scaffold.docx'),
                 os.path.join(SCRATCH, OUT_NAME),
                 body_xml,
                 extra_media={n: os.path.join(SCRATCH, n) for n in FIGS})
