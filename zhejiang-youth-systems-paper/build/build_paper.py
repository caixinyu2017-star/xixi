# -*- coding: utf-8 -*-
"""Assemble the YSTS manuscript as an MDPI Systems .docx."""
import os
import re
import shutil
import sys
...
from mdpi_builder import (CITE_RE, build_docx, p_body, p_h1, p_h2, p_h3, p_equation,
                          p_table_caption, p_fig_caption, p_notes, p_backmatter,
                          p_reference, p_figure, _tc, _borders, wrun)
from ysts_equations import EQUATIONS
from ysts_content_a import PART_A, TITLE, ABSTRACT, KEYWORDS
from ysts_content_b import PART_B
from ysts_refs import REFS
...
# ---------------------------------------------------------- body
FIG_FILES = [f'fig{i}_{n}.png' for i, n in enumerate(
    ['cld', 'stockflow', 'baseline', 'structure', 'sensitivity', 'policy',
     'archetypes'], start=1)]
...
for b in BLOCKS:
    ...
    if kind == 'h1':
        ...
        parts.append(p_figure(FIG_RIDS[b[1]], cx, cy, b[4])); n_fig += 1
    elif kind == 'figcap':
