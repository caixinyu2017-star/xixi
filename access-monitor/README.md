...
> **挂后台之前一定要装好 ddddocr。** 任务计划里跑的程序没有终端，
> 「请人工输验证码」这条路走不通（程序会检测到并直接报错退出，不会卡死，但也就不监控了）。
> 装了 ddddocr 才能真正无人值守。
...
关于「宁可不报也不误报」：程序抓到页面后会先确认上面有「最近访问记录 / 来访IP / 访问时间」这类特征词，
确认不了就**整轮跳过**，绝不硬解析。因为解析器最后一层兜底是全文正则捞 IP，
拿它去啃一个错误页，捞出来的垃圾记录时间戳全是「此刻」，会立刻凑够 3 条报一次假警——
这是最需要避免的失败模式。如果你们后台用词不一样导致一直跳过，
改 `config.yaml` 的 `navigation.page_markers` 就行，日志里会写清楚。
