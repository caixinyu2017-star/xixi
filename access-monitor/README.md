...
而邮件在手机上常常延迟几分钟才到，不适合当唯一的实时通道。

### 首选：即时推送（秒到）

| 通道 | 适合谁 | 怎么拿 key | 额度 |
| --- | --- | --- | --- |
| **企业微信群机器人** ⭐ | 安卓 / iPhone 都行，最省事 | 电脑版企业微信 → 右键目标群 → 添加群机器人 → 新建 → 复制 webhook 里 `key=` 后面那段。**可以自己建一个只有自己的群** | 20 条/分钟，无每日上限 |
| **Bark** ⭐ | iPhone 用户 | App Store 装 Bark，打开就能复制 key | 无上限 |
| 钉钉自定义机器人 | 单位用钉钉 | 群设置 → 智能群助手 → 添加机器人 → 安全设置选「加签」，token 和 secret 都填上 | 20 条/分钟 |
| 飞书自定义机器人 | 单位用飞书 | 群设置 → 群机器人 → 添加自定义机器人 | 100 条/分钟 |
| Server酱 | 想用微信收 | https://sct.ftqq.com 微信扫码即得 SendKey | **免费版每天只有 5 条** |

Bark 的两个小细节：程序默认用 `timeSensitive` 级别（能穿透专注模式），严重告警才用 `critical`（能突破静音，
但需要你在 iOS 通知设置里单独给 Bark 授权「重要警告」）；同一次突发会**更新同一条通知**而不是在锁屏上堆一长串。
...
邮件是 HTML 的，每个 IP 一张小卡片——位置、运营商、AS、标签、地图链接、完整访问明细表都在里面，
这些是 IM 机器人显示不下的。

> **不要用 Outlook / 微软账号发信**：微软已经在 2025 年停用了 SMTP 的账号密码登录，
> 现在必须走 OAuth2，`smtplib` 填账号密码是连不上的。Gmail 可以用，但要先开两步验证生成应用专用密码，
> 而且大陆网络一般需要代理。

配好之后跑 `python run.py test-notify` 验证。程序会分别报告每个通道成功与否——
注意企业微信、钉钉、飞书、PushPlus 都是「HTTP 200 但响应体里带错误码」，所以程序检查的是响应体而不是状态码。
