...
_GREEK = {'alpha':'α','beta':'β','gamma':'γ','delta':'δ','epsilon':'ε',
          ...
          'times':'×','cdot':'·','prime':'′','infty':'∞','leq':'≤','geq':'≥',
          'approx':'≈','partial':'∂'}

_SCRIPT = {'A':'𝒜','B':'ℬ','E':'ℰ','F':'ℱ','G':'𝒢','H':'ℋ','I':'ℐ','L':'ℒ',
           'M':'ℳ','N':'𝒩','P':'𝒫','R':'ℛ','S':'𝒮','V':'𝒱','W':'𝒲'}

# an inline spec is a sequence of atoms, each optionally carrying _{..} / ^{..}
_ATOM = re.compile(
    r'\\bar\{(?P<bar>[^}]*)\}'
    r'|\\mathcal\{(?P<cal>[^}]*)\}'
    r'|\\(?P<cmd>[A-Za-z]+)'
    ...
    r'|(?P<name>[A-Za-z]+)'
    r'|(?P<other>.)')
_SCRIPT_SUFFIX = re.compile(r'(?:_\{(?P<sub>[^}]*)\})?(?:\^\{(?P<sup>[^}]*)\})?')

# characters that Word sets upright inside a formula
_UPRIGHT = set('=+−-<>≤≥≈()[],;/|·×∂ ')
...
def _script(base, sub, sup):
    if sub and sup:
        return ('<m:sSubSup><m:sSubSupPr/>'
                f'<m:e>{base}</m:e><m:sub>{_atoms(sub)}</m:sub>'
                f'<m:sup>{_atoms(sup)}</m:sup></m:sSubSup>')
    if sub:
        return msub(base, _atoms(sub))
    if sup:
        return msup(base, _atoms(sup))
    return base


def _atoms(spec):
    """Tokenise an inline spec into a run of OMML nodes."""
    out, pos = [], 0
    while pos < len(spec):
        m = _ATOM.match(spec, pos)
        pos = m.end()
        if m.group('bar') is not None:
            base = mbar(_atoms(m.group('bar')))
        elif m.group('cal') is not None:
            c = m.group('cal')
            base = mr(_SCRIPT.get(c, c))
        elif m.group('cmd') is not None:
            base = mr(_GREEK.get(m.group('cmd'), m.group('cmd')))
        elif m.group('num') is not None:
            base = mr(m.group('num'), upright=True)
        elif m.group('name') is not None:
            base = mr(m.group('name'))
        else:
            c = m.group('other')
            c = '−' if c == '-' else c
            out.append(mr(c, upright=c in _UPRIGHT))
            continue
        s = _SCRIPT_SUFFIX.match(spec, pos)
        pos = s.end()
        out.append(_script(base, s.group('sub'), s.group('sup')))
    return ''.join(out)


def inline_math(spec):
    """Parse an inline-math spec into OMML.  Handles greek names, sub- and
    superscripts, \\bar{} and \\mathcal{}, and sequences of these joined by
    operators.  Variable codes containing a bare underscore (e.g. EDL_SPEC)
    are emitted as italic text runs — they are labels, not formulas, and a
    literal '_' inside m:t breaks LibreOffice's OMML import."""
    spec = spec.strip()
    if '_' in spec and '_{' not in spec:
        return wrun(spec, sz=20, i=True)
    return omath(_atoms(spec))
