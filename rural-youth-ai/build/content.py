...
BLOCKS = [
    ...
    ("p",
     ...
     "which wrong model of the world generated the data. Second, the generative-AI "
     "profiling layer was degraded by re-drawing each profile "
     "field with error rates up to 30 per cent. Stay-yield falls "
     f"from {f2(nz_at(0.0)['ramt'])} at a clean profile to "
     f"{f2(nz_at(0.3)['ramt'])} at 30 per cent field error — "
     "profiling quality is a first-order determinant of allocation "
     "quality, and the platform baseline degrades in parallel "
     f"(from {f2(nz_at(0.0)['mlp'])} to "
     f"{f2(nz_at(0.3)['mlp'])}), so better matching never "
     "substitutes for better extraction. For a county this is an "
     "argument to audit the upstream layer with the same "
     "seriousness as the matcher, a duty the transparency "
     "provisions of the generative-AI measures already gesture at "
     "[[migliorini2024]]."),
    ("table", "table4"),
