...
6. **图**：`{'fig': 'figs/figN_M.png', 'caption': '图N.M  中文图名', 'width_cm': 13.5, 'note': '注：……。数据来源：……。'}`。**每幅图必须带 note**（图题下方、小五宋体、无缩进、两端对齐，与表注同格式）：分析图注明口径与数据来源；机制图写“注：（符号/要点说明）。资料来源：作者绘制。”注文严禁写进图片内部。只能用 manifest 分配给本章的图；正文必须先提及“如图N.M所示”，图块紧随该段之后，且图后有解读。
7. **表**：`{'table': {'caption': '表N.M  中文表名', 'header': [...], 'rows': [[...]], 'note': '注：…。数据来源：…。', 'col_align': ['left','center',...]}}`。表名在上；估计表数值从 R 取（cell/coef 格式化）；每表正文先提及后出现、并有解读。
...
assert all(x.get('note') for x in figs), '存在缺少 note 的图'
assert all(x['table'].get('note') for x in tabs), '存在缺少 note 的表'
