...
TEMPLATE = os.path.join(HERE, "behavscitemplate.dot")
OUT = os.path.join(
    ROOT,
    "Between_and_Within_Person_Career_Moderation_BehavSci_manuscript.docx")
...
_K = r"[a-zA-Z0-9_\-]+"
_GRP = r"\[\[" + _K + r"(?:\],\[" + _K + r")*\]\]"
TOKEN = re.compile(r"(\$[^$]*\$|" + _GRP + r"(?:,\s*" + _GRP + r")*)")
CITE = re.compile(_K)
...
def add_rich(paragraph, text, size=None, italic=False, bold=False):
    ...
    for seg in TOKEN.split(text):
        ...
        if seg.startswith("$") and seg.endswith("$") and len(seg) > 1:
            ...
            # Behavioral Sciences uses APA author-date citations, not numbers.
            # A key ending in "|n" is rendered narratively, as Author (Year);
            # everything else is parenthetical, and a group of keys becomes a
            # single parenthesis with the entries separated by semicolons and
            # ordered alphabetically, as APA requires.
            keys = CITE.findall(seg)
            narrative = [k for k in keys if k.endswith("_n")]
            plain = [k.rstrip() for k in keys]
            r = paragraph.add_run(R.render_citation(plain))
            if size:
                r.font.size = Pt(size)
            for k in plain:
                cite_number(k.removesuffix("_n"))
        else:
...
# ---------------------------------------------------------------------------
def render_reference(p, key, number=None):
    """Render one reference in APA 7 style, as Behavioral Sciences requires."""
    ref = R.REFS[key]

    def txt(t, italic=False, bold=False):
        run = p.add_run(_typo(t))
        run.font.size = Pt(9)
        run.font.italic = italic
        run.font.bold = bold

    kind = ref.get("kind", "article")
    _a = R.author_list(ref)
    # an institutional author carries no initials, so it needs its own stop
    txt(_a + ("" if _a.endswith(".") else ".") + " ")
    txt("(%s). " % ref["year"])

    if kind == "article":
        _t = R.sentence_case(ref["title"])
        txt(_t + ("" if _t[-1] in "?!" else ".") + " ")
        ...
        txt(R.sentence_case(ref["title"]), italic=True)
        if ref.get("edition"):
            txt(" (%s)" % ref["edition"])
        txt(". " + ref["publisher"] + ".")
    elif kind == "chapter":
        txt(R.sentence_case(ref["title"]) + ". In " + ref["editors"] + " (Ed"
            + ("s" if ";" in ref["editors"] or "&" in ref["editors"] else "")
            + ".), ")
        txt(R.sentence_case(ref["booktitle"]), italic=True)
        if ref.get("pages"):
            txt(" (pp. %s)" % ref["pages"])
        txt(". " + ref["publisher"] + ".")
    else:                                    # report, dataset, software, web
        txt(R.sentence_case(ref["title"]), italic=True)
        txt(". " + ref.get("publisher", "") + ".")
        if ref.get("url"):
            txt(" " + ref["url"])
...
def body(doc):
    ...
    for block in C.BLOCKS:
        ...
        if kind == "h1":
            ...
            p_ = doc.add_paragraph(style="MDPI_2.1_heading1")
            p_.paragraph_format.space_before = Pt(14)
            p_.add_run(block[1])
        elif kind == "h2":
...
def _stamp_volume(path):
    """The template ships the previous volume in its header and footer."""
    import shutil
    import zipfile
    tmp = path + ".tmp"
    with zipfile.ZipFile(path) as zin, \
        ...
        for item in zin.infolist():
            ...
            if item.filename.startswith(("word/header", "word/footer")):
                blob = blob.replace(b">2025<", b">2026<")
                blob = blob.replace(b"> 15<", b"> 16<")
