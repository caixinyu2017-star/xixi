...
def three_line_table(doc, header, rows, widths_cm, size=WU,
    ...
    # 固定列宽：同时写 tblW 与 tblGrid，否则 LibreOffice/WPS 会按内容重新分配
    total = int(sum(widths_cm) * 567)          # cm -> twips
    tw = tblPr.find(qn("w:tblW"))
    if tw is None:
        tw = OxmlElement("w:tblW")
        tblPr.insert_element_before(tw, "w:jc", "w:tblCellSpacing", "w:tblInd",
    ...
    if grid is not None:
        for col, wcm in zip(grid.findall(qn("w:gridCol")), widths_cm):
            col.set(qn("w:w"), str(int(wcm * 567)))
