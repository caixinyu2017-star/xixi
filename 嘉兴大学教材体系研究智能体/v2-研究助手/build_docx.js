...
children.push(new Paragraph({
  ...
  children: [new TextRun({ text: '“禾　研”', font: HEI, size: 52, bold: true, color: BLACK })],
}));
children.push(new Paragraph({
...
while (i < raw.length) {
  ...
  if (/^## /.test(t)) {
    ...
    const isChapter = /^[一二三四五六七八九十]+、/.test(title) || /^第.+章/.test(title) || ['摘要', '结语', '附录'].some(k => title.startsWith(k));
    ...
    headers: {
      ...
      default: new Header({
        children: [new Paragraph({
          ...
          children: [new TextRun({ text: '「禾研」中国特色高质量教材体系研究助手建设方案', font: HEI, size: 18, color: BLACK })],
        })],
      }),
    },
    footers: {
