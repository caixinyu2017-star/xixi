...
ABSTRACT = (
    ...
    "framework. The motivation stems from a practical difficulty in "
    "clinical and research psychology: ecological momentary assessment "
    "now yields dense records of how people actually regulate their "
    "emotions, yet the models that summarise those records into "
    "regulation profiles give the clinician no account of which "
    "moments produced a given conclusion. Our methodology integrates "
    "two parallel encoding streams. A Temporal Affect Graph Network "
    "encodes the sequence of momentary reports as a banded temporal "
    "graph in which nodes are episodes, edges express short-range "
    "dependence between them, and a gated propagation mechanism "
    "carries affective context across neighbouring moments. In "
    "parallel, a Regulatory Process Ontology Parser translates the "
    "process model of emotion regulation, expressed as a directed "
    "acyclic ontology over five strategy families and their "
    "superordinate classes, into differentiable concept vectors by "
    "graph attention over interpretable node descriptors. The two are "
    "coupled by a concept-conditioned fusion whose score for each "
    "family decomposes exactly into the contributions of the "
    "individual moments, so that every score arrives with the moments "
    "that justify it. Because the corpus is simulated, the strategy "
    "actually deployed at each prompt is known, and that evidence "
    "trail can be checked rather than merely asserted. On a "
    "psychologically calibrated corpus of "
    f"{CFG['n_participants']} participants, each contributing up to "
    f"{CFG['t_max']} momentary reports over fourteen days, DP-APT "
    f"attains an AUC-ROC of {f3(OURS['auc_roc'])} against "
    f"{f3(BB['auc_roc'])} for the strongest sequence baseline, and its "
    f"trail reaches an alignment of {f3(OURS['alignment'])} where "
    f"chance is {f3(CORP['chance_alignment'])} and a supervised probe "
    f"of the same features reaches {f3(CORP['oracle_alignment'])} — "
    "the best of any model evaluated. The principal finding is "
    "negative and general: across models, and within one model as its "
    "attention is sparsified, the established faithfulness criteria "
    "and the correctness of the explanation move in opposite "
    "directions. Explanations that are faithful to the model are "
    "therefore not thereby correct about the person, and the "
    "significance of this work lies in showing that the difference "
    "can be measured and that it is large.")
...
BLOCKS = [
    ...
    ("p",
     ...
     "architecture with the strongest erasure scores has almost the "
     "weakest alignment, while post-hoc attributions on an "
     "unconstrained encoder, which the erasure tests rate poorly, "
     "point at the right moments far more often. Within a single "
     "model, sparsifying its attention — the standard way to make an "
     "attention explanation legible — improves the erasure scores "
     "monotonically while alignment with what the person actually did "
     "declines. Faithfulness certifies that a model used the evidence "
    ...
    ("eq", r"f: (x_{i,1:T}, m_{i,1:T}) \rightarrow "
           r"(\hat{y}_{i}, C_{i})", 1),
    ("where",
     "where $\\hat{y}_{i} \\in [0,1]^{K}$ is the predicted profile and "
    ...
    ("where",
     ...
     f"{f2(COLLAPSE['without residual']['max_abs_cosine'])}, which is "
     "two families the detector cannot tell apart; the skip connection "
     f"brings that down to "
     f"{f2(COLLAPSE['with residual']['max_abs_cosine'])}. It does not "
     "change the average similarity between the five vectors, which "
     f"stays near "
     f"{f2(COLLAPSE['with residual']['mean_abs_cosine'])} either way "
     "and reflects the fact that the descriptors place all five "
    ...
    ("where",
     ...
     "an explanation that spreads its weight thinly over sixty episodes "
     "is not usable by a clinician, this variant admits the "
    ...
    ("p",
     ...
     f"compliance is set to 85 per cent, the level typically achieved "
    ...
    ("p",
     ...
     "whose operations are verified against central finite differences "
     "to a maximum relative deviation below "
     "$10^{-6}$. The episode and concept representations are "
     ...
     "comparison is not confounded by tuning. Every reported figure "
     "is the mean over "
    ...
    ("p",
     ...
     "behind it. We report the shortfall rather than obscuring it. It "
     "is the price of a read-out constrained to decompose into "
     "per-episode terms, and the constraint is what the rest of the "
     "paper is about. Against the interpretable comparison model, the "
     f"attentive diagnostic architecture, DP-APT is ahead by "
     f"{f3(OURS['auc_roc'] - MOD['Attentive diagnostic model']['auc_roc'])}"
     " AUC-ROC points, indicating that the symbolic pathway "
     "contributes accuracy as well as structure."),
    ("table", "table1"),
    ...
    ("p",
     ...
     "any model evaluated. Its comprehensiveness is "
    ...
    ("p",
     ...
     "ordering. The attentive diagnostic model has the strongest "
     f"comprehensiveness in the table "
     f"({f3(MOD['Attentive diagnostic model']['comprehensiveness'])}) "
     f"and nearly the weakest alignment "
     f"({f3(MOD['Attentive diagnostic model']['alignment'])}). The "
     "integrated-gradient explanations of the gated recurrent encoder "
     f"have the weakest comprehensiveness "
     ...
     "faithfulness. Across the models in the table the two criteria "
     "are not merely uncorrelated; they run in opposite directions. An "
     "explanation can point firmly at episodes the model depends on "
     "and still be pointing at the wrong episodes, and it can point at "
     "the right episodes while the model does not depend on them "
     "much."),
    ("p",
     "The row for the attention variant of our own model makes the "
     "point without changing the encoder, the data or the training "
     "objective. Read through its attention distribution, that model "
     f"attains a comprehensiveness of "
     f"{f3(ABL['with attention over episodes']['comprehensiveness'])} "
     "— it passes the erasure test — while its alignment is "
    ...
    ("p",
     ...
     "AUC-ROC points and halves the alignment of the resulting "
     f"evidence trail, from {f3(ABL['full model']['alignment'])} to "
     f"{f3(ABL['with attention over episodes']['alignment'])}. "
    ...
    ("p",
     ...
     f"rises steadily, from {f3(SW0['comprehensiveness'])} at "
     f"$\\lambda = 0$ to {f3(SW_MAX['comprehensiveness'])} at "
     ...
     f"detect. Over the same range accuracy falls from "
     f"{f3(SW0['auc_roc'])} to {f3(SW_MAX['auc_roc'])}, and alignment "
     f"falls from {f3(SW0['alignment'])} to "
     f"{f3(SW_MAX['alignment'])}."),
    ("p",
     "A practitioner tuning $\\lambda$ upward would watch the standard "
     "faithfulness measure improve and would conclude that the "
     "explanations were getting better, while the share of highlighted "
     "moments that the person really spent regulating in the named way "
     "was going down. Faithfulness asks whether the model used the "
     "evidence it points at; alignment asks whether the evidence it "
     "points at is the right evidence. Optimising the first does not "
     "deliver the second, and here it works against it."),
    ("fig", "fig4"),
    ...
    ("p",
     ...
     f"free. DP-APT concedes {f3(BB['auc_roc'] - OURS['auc_roc'])} "
     "AUC-ROC points to an unconstrained sequence encoder. The reason "
     "is structural rather than incidental. A regulation profile is "
    ...
    ("p",
     ...
     f"{f2(OURS['alignment'] / CORP['chance_alignment'])} times better "
     "aligned with what the person actually did is worth "
     f"{f3(BB['auc_roc'] - OURS['auc_roc'])} of AUC."),
    ...
    ("p",
     ...
     "resulting confusion. Across models the two criteria are "
     "inversely ordered, so a reader ranking explanations by "
     "comprehensiveness would rank them almost exactly wrongly by "
     "correctness. And within one model, raising the weight on the "
     "entropy penalty — the obvious way to make an attention "
     "explanation more concentrated, and one that improves "
     "comprehensiveness monotonically — moves alignment in the "
     "opposite direction."),
    ("p",
    ...
    ("p",
     ...
     "are inversely ordered, and within one model tuning for "
     "faithfulness makes the trail less correct rather than more."),
    ("p",
