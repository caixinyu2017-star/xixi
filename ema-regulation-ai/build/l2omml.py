...
OPS = {
    ...
    'ell': 'ℓ', 'propto': '∝', 'equiv': '≡', 'forall': '∀', 'exists': '∃',
    'odot': '⊙', 'oplus': '⊕', 'otimes': '⊗', 'top': '⊤', 'bot': '⊥',
    'leftarrow': '←', 'gets': '←', 'mapsto': '↦', 'subset': '⊂',
...
def _atom(t):
    ...
    if c == '\\':
        ...
        if name in ('hat', 'bar', 'tilde', 'vec', 'dot', 'overline',
            ...
            chr_map = {'hat': '^', 'bar': '̅', 'overline': '̅', 'tilde': '~',
                       'vec': '⃗', 'dot': '̇', 'mathring': '̊',
                       'check': '̌', 'breve': '̆'}
        ...
        if name == 'mathbb':
            inner = t.read_group()
            return [_run("".join(BLACKBOARD.get(ch, ch) for ch in inner))]
        if name in ('mathcal', 'mathscr'):
            # Script capitals live in the Unicode astral plane and are not
            # present in the body font, so calligraphic sets are set as
            # plain italic capitals, the convention the journal's own
            # published equations follow.
            return [_run(t.read_group())]
        if name == 'sum' or name == 'prod' or name == 'int':
            ...
            # consume the remainder of the current stream as the summation operand
...
# ---------------------------------------------------------------------------
# String interfaces used by the raw-XML document builder.
# ---------------------------------------------------------------------------
from lxml import etree as _etree      # noqa: E402

_M_NS = ('xmlns:m="http://schemas.openxmlformats.org/officeDocument/'
         '2006/math"')


def _ser(el):
    s = _etree.tostring(el, encoding="unicode")
    # strip namespace declarations that the parent already carries
    s = re.sub(r'\s+xmlns:[a-zA-Z0-9]+="[^"]*"', "", s)
    return s


def math_body(latex):
    """OMML content (no wrapper) for a display equation."""
    return "".join(_ser(e) for e in parse(latex))


def inline(latex):
    """An inline <m:oMath> element for use inside a text paragraph."""
    return "<m:oMath>%s</m:oMath>" % math_body(latex)
