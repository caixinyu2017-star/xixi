...
ABSTRACT = (
    "Generative artificial intelligence is most capable at exactly the "
    "tasks from which early careers have traditionally been built, and the "
    "graduates now entering the labor market are the first cohort to choose "
    "careers under that condition. This study models the fresh graduate as "
    "an adaptive agent inside a socio-technical labor system: the perception "
    "that one's human capital is depreciating is the input signal, "
    "AI-related career anxiety is the error signal of a cybernetic control "
    "loop, and occupational adaptation and career avoidance are the two "
    "behavioral outputs through which the loop closes. Hypotheses are tested "
    f"with a two-wave survey of {S['n_t1']:,} Chinese graduates within three "
    f"years of graduation ({S['n_t2']:,} matched across waves), analyzed by "
    "confirmatory factor analysis, bootstrapped conditional-process models "
    "and fuzzy-set qualitative comparative analysis. Perceived depreciation "
    f"raises anxiety strongly (β = {n('a1')}). Anxiety converts into "
    "adaptation along an inverted U: the response peaks "
    f"{TP['tau']:.2f} standard deviations above the mean, and "
    f"{n('share_beyond', 1)} percent of graduates are already beyond the "
    "point at which additional anxiety buys additional adaptation, while "
    f"avoidance rises monotonically (β = {n('c1')}). Generative-AI literacy "
    f"shifts the turning point from {S['tp_lit'][0]['tau']:.2f} to "
    f"{S['tp_lit'][2]['tau']:.2f} standard deviations and employability "
    f"support cuts the depreciation-to-anxiety pass-through from "
    f"{S['fslo']['est']:.2f} to {S['fshi']['est']:.2f}. The configurational "
    "analysis finds exactly one sufficient recipe for high adaptation, the "
    "conjunction of anxiety, literacy and support (consistency "
    f"{S['qca']['sol_cons']:.3f}); anxiety without its complements never "
    "suffices. Anxiety is thus a productive control signal only when the "
    "system supplies the capability and the support to act on it, which "
    "relocates the policy lever from calming graduates down to completing "
